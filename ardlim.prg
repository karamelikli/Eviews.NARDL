	...
	'This code has been written by Huseyin Karamelikli (Hossein Haghparast Gharamaleki) Thanks to Yashar Tarverdi. Him works is expanded here in some directions'
	'For any questions or comments please contact hakperest@gmail.com '
	'@npers
	'---------------------------------------------------------------------------'
	%ValidFrom="01/01/17" 'mm/dd/yy
	%ValidUntil="08/01/22"
	
'@uiprompt( @str(@qfdist(0.90,10,2)))
'@uiprompt( @str(@qchisq(0.90,1)))
	
	'stop
	
	
	if %ValidFrom and  %ValidUntil then	
	    !ValidFrom=0
	    !ValidUntil=0
	    !simdi=0
	    call DateToInt(%ValidFrom,!ValidFrom)
	    call DateToInt(@date,!simdi)
	    call DateToInt(%ValidUntil,!ValidUntil)
	    if !simdi<!ValidFrom or !simdi>!ValidUntil then
	      	    @uiprompt("You have a serious problem now. Please redownload HARDL from bitbucket.org")
	      	    stop
	    endif
	endif		
